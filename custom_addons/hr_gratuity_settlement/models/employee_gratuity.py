...
class EmployeeGratuity(models.Model):
    ...
    allowance = fields.Char(string="Extra Allowance", default=0)
    gratuity_amount = fields.Integer(string="Gratuity Payable", required=True, default=0, readony=True, help=("Gratuity is calculated based on the "
    ...
    def check_request_existence(self):
        for rec in self.filtered(lambda x: x.employee_name):
            if rec.employee_name:
                gratuity_request = self.env['hr.gratuity'].search([('employee_name', '=', rec.employee_name.id), ('state', 'in', ['draft', 'validate', 'approve', 'cancel'])], limit=1)
                if gratuity_request:
