...
class RevenueKPIsDashboard(http.Controller):
    ...
    def cohort(self, date_start, cohort_period, cohort_interest, filters):
        ...
        if filters.get('additional_field'):
            subs_domain.append(('your_field', '=', filters.get('additional_field')))

        for cohort_group in request.env['sale.order']._read_group_raw(
                domain=subs_domain, fields=['date_start'], groupby='date_start:' + cohort_period):
            tf = cohort_group['date_start:' + cohort_period]
            date1 = tf[0].split('/')[0]
            cohort_subs = request.env['sale.order'].search(cohort_group['__domain'])
            cohort_date = datetime.strptime(date1, DEFAULT_SERVER_DATE_FORMAT)

            if cohort_interest == 'value':
                starting_value = float(sum([x.currency_id.compute(x.recurring_monthly,
                                                                  company_currency_id) if x.currency_id else x.recurring_monthly
            ...
            for ij in range(0, 16):
                ...
                churned_subs = [x for x in cohort_subs if
                                x.date and datetime.strptime(x.date, DEFAULT_SERVER_DATE_FORMAT).strftime(
                                    DISPLAY_FORMATS[cohort_period]) == significative_period]

                if cohort_interest == 'value':
                    churned_value = sum([x.currency_id.compute(x.recurring_monthly,
                                                               company_currency_id) if x.currency_id else x.recurring_monthly
                                         for x in churned_subs])
                else:
        ...
        return {
            'contract_templates': request.env['sale.order.template'].search_read([], ['name']),
    ...
    def fetch_data(self):
        ...
        return {
            ...
            'has_def_revenues': bool(request.env['sale.order.template'].search([]).mapped('template_asset_category_id')),
    ...
    def _get_default_values_forecast(self, forecast_type, end_date, filters):
    ...
    def _get_stats_history(self, stat_type, start_date, end_date, filters):
    ...
    def _get_stats_by_plan(self, stat_type, start_date, end_date, filters):
        ...
        for template in template_ids:
            sale_subscriptions = request.env['sale.order'].search([('template_id', '=', template.id), ('is_subscription', '=', True)])
