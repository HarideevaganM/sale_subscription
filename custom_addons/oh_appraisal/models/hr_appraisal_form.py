    ...
    def action_start_appraisal(self):
        ...
        for appraisal_reviewers, survey_id in appraisal_reviewers_list:
            for reviewers in appraisal_reviewers:
                ...
                if token:
                    url = url + '/' + token
                    mail_content = "Dear " + reviewers.name + "," + "<br><br>Please fill out the following survey " \
                                   "related to " + self.emp_id.name + "<br><br>Click here to access the survey.<br>" + \
                                   str(url) + "<br>Post your response for the appraisal on or before " + str(self.appraisal_deadline)
