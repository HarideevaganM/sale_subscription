...
class HrLeaveRequest(models.Model):
    ...
    def _get_overlapping_leaves(self):
        if self.type == 'remove' and self.date_from and self.date_to:
            ...
            for leave in other_leaves:
                frm_dte = datetime.strptime(leave.date_from, '%Y-%m-%d').date()
                to_dte = datetime.strptime(leave.date_to, '%Y-%m-%d').date()
    ...
    #@api.one
    def _get_hr_holiday_managers(self):
...
class PendingTask(models.Model):
    ...
    #@api.one
    def _get_unavailable_employee(self):
        unavail_emp = []
