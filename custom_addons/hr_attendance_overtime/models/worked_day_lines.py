...
class Contract(models.Model):
    ...
    work_hours = fields.Float(string='Working Hours', default=8.0)
    ...
    def _get_worked_day_lines(self, contract_ids, date_from, date_to):
        ...
        for contract in self.env['hr.contract'].browse(contract_ids):
            ...
            for day in range(0, nb_of_days):
                working_intervals_on_day = contract.work_hours.get_working_intervals_of_day(
        ...
        date_to = str(date_to)
        for contract in self.env['hr.contract'].browse(contract_ids):
            attendance_ids  = self.env['hr.attendance'].search([('check_in', '>=', date_from), ('check_out', '<=', date_to)])
            for attn_lines in attendance_ids:
