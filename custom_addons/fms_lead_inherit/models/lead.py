...
class CrmLeadInherit(models.Model):
    ...
    #~ document_capture=fields.Binary(string='Customer Details',store=True)
    ...
    # PO details removed #
    #~ po_capture=fields.Binary(string='PO Details',store=True)
    # Return date changes ## 
    ...
    def compute_sale(self):
        for rec in self:
            rec.sale_number = self.env['sale.order'].search([('opportunity_id','=',rec.id)])
...
# for onchange from res.partner to crm.lead
    ...
    def compute_customer_type(self):
        for rec in self:
            if rec.partner_id:
                rec.customer_types = rec.partner_id.customer_types
            else:
                rec.customer_types = False
