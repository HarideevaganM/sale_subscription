...
class WebsiteSupportDepartment(models.Model):
    ...
    def _compute_sub_category_week_ids(self):
        for rec in self:
        #Unlink all existing records
            for si in self.env['website.support.department.subcategory'].search([('wsd_week_id', '=', rec.id)]):
                si.unlink()
            ...
            for sub_category in self.env['website.support.ticket.subcategory'].search([]):
                ...
                if count > 0:
                    self.env['website.support.department.subcategory'].create( {'wsd_week_id': rec.id, 'subcategory_id': sub_category.id, 'count': count} )

            rec.sub_category_week_ids = self.env['website.support.department.subcategory'].search([('wsd_week_id', '=', rec.id)], limit=5)
...
class WebsiteSupportDepartmentContact(models.Model):
    ...
    def _default_role(self):
        ...
        return self.env.ref('website_support.website_support_department_manager')
