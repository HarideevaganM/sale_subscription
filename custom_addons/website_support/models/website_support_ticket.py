...
class WebsiteSupportTicket(models.Model):
    ...
    def _read_group_state(self, states, domain, order):
        ...
        staff_replied_state = self.env.ref('website_support.website_ticket_state_staff_replied')
        ...
        customer_closed = self.env.ref('website_support.website_ticket_state_customer_closed')
        # staff_closed = self.env['ir.model.data'].get_object('website_support', 'website_ticket_state_staff_closed')
        staff_closed = self.env.ref('website_support.website_ticket_state_staff_closed')
    ...
    def _default_state(self):
        ...
        return self.env.ref('website_support.website_ticket_state_open')
    ...
    def _default_approval_id(self):
        ...
        return self.env.ref('website_support.no_approval_required')
    ...
    def update_sla_timer(self):
        ...
        for active_sla_ticket in self.env['website.support.ticket'].search([('sla_active','=',True),('sla_id','!=',False),('sla_rule_id','!=',False)]):
            ...
            notification_template = self.env.ref('website_support.support_ticket_sla_alert')
            ...
            for sla_alert in self.env['website.support.sla.alert'].search([('vsa_id','=',active_sla_ticket.sla_id.id), ('alert_time','>=', active_sla_ticket.sla_timer)]):
                ...
                if sla_alert not in active_sla_ticket.sla_alert_ids:
                    ...
                    for my_user in active_sla_ticket.category_id.cat_user_ids:
                        fields = ['body_html', 'email_to']
                        values = notification_template.generate_email(active_sla_ticket.id, fields)
    ...
    def message_new(self, msg, custom_values=None):
        ...
        setting_email_default_category_id = self.env['ir.default'].sudo().get('website.support.settings', 'email_default_category_id')
    ...
    def message_update(self, msg_dict, update_vals=None):
        ...
        if msg_dict.get('to') == self.email:
            ...
            change_state = self.env.ref('website_support.website_ticket_state_staff_replied')
        else:
            # change_state = self.env['ir.model.data'].get_object('website_support','website_ticket_state_customer_replied')
            change_state = self.env.ref('website_support.website_ticket_state_customer_replied')
    ...
    def request_approval(self):
        ...
        approval_email = self.env.ref('website_support.support_ticket_approval')
        fields = ['body_html']
        values = self.env['mail.compose.message'].generate_email_for_composer(approval_email.id, [self.id], fields)[self.id]
    ...
    def _needaction_domain_get(self):
        ...
        open_state = self.env.ref('website_support.website_ticket_state_open')
        # custom_replied_state = self.env['ir.model.data'].get_object('website_support', 'website_ticket_state_customer_replied')
        custom_replied_state = self.env.ref('website_support.website_ticket_state_customer_replied')
    ...
    def create(self, vals):
        ...
        for my_user in new_id.category_id.cat_user_ids:
            fields = ['body_html', 'email_to']
            values = notification_template.generate_email(new_id.id, fields)
    ...
    def write(self, values, context=None):
        ...
        if 'category_id' in values:
            ...
            change_category_email = self.env.ref('website_support.new_support_ticket_category_change')
        ...
        if 'user_id' in values:
            ...
            if setting_change_user_email_template_id:
                ...
                email_template = self.env.ref('website_support.support_ticket_user_change')
            fields = ['model', 'res_id', 'user_id', 'email_to', 'body_html', 'body']
            email_values = email_template.generate_email([self.id], fields)[self.id]
    ...
    def send_survey(self):
        ...
        notification_template = self.env.ref('website_support.support_ticket_survey')
        fields = ['model', 'email_from', 'email_to', 'subject', 'body_html']
        # print("---", self.id, "--self.id-\n")
        values = notification_template.generate_email(self.id, fields)
        send_mail = self.env['mail.mail'].create(values)
        send_mail.send(True)
...
class WebsiteSupportTicketMessage(models.Model):
    ...
    def create(self, values):
        ...
        if values['by'] == "customer":
            ...
            customer_reply_email_template = self.env.ref('website_support.support_ticket_customer_reply_wrapper')
            fields = ['email_to']
            email_values = customer_reply_email_template.generate_email(new_record.id, fields)
...
class WebsiteSupportTicketClose(models.TransientModel):
    ...
    def _onchange_template_id(self):
        if self.template_id:
            fields = ['body_html']
            values = self.env['mail.compose.message'].generate_email_for_composer(self.template_id.id, [self.ticket_id.id], fields)[self.ticket_id.id]
    ...
    def close_ticket(self):
        ...
        setting_auto_send_survey = self.env['ir.default'].sudo().get('website.support.settings', 'auto_send_survey')
...
class WebsiteSupportTicketCompose(models.Model):
    ...
    def send_reply(self):
        ...
        if self.approval:
            ...
            awaiting_approval_state = self.env.ref('website_support.website_ticket_state_awaiting_approval')
        ...
        setting_staff_reply_email_template_id = self.env['ir.default'].sudo().get('website.support.settings', 'staff_reply_email_template_id')
        ...
        fields = ['model', 'email_to', 'email_from', 'subject', 'body_html']
        ...
        if self.approval:
            ...
            awaiting_approval = self.env.ref('website_support.awaiting_approval')
            ...
            staff_replied = self.env.ref('website_support.website_ticket_state_staff_replied')
