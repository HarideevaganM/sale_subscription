...
class MonthlyAttendanceReport(models.Model):
    _name = 'report.bbis_attendance_temp.monthly_attendance_report'
    ...
    def _get_in_out(self, emp_id, data):
        ...
        self.env.cr.execute(
            ...
            LEFT JOIN (
                SELECT employee_id, create_date, check_in, check_out, remarks,
    ...
    def generate_xlsx_report(self, workbook, data, orders):
        ...
        title_format = workbook.add_format(
            {'bold': True, 'border': True, 'align': 'center', 'valign': 'vcenter', 'font_size': 13})
        header_format = workbook.add_format(
            {'bold': True, 'border': True, 'font_size': 11, 'align': 'center', 'valign': 'vcenter',
             'bg_color': '#FFFF00', 'font_color': 'black'})
        cell_format_center = workbook.add_format(
            {'valign': 'vcenter', 'border': True, 'font_size': 11, 'align': 'center', 'text_wrap': True})
        ...
        for emp in employees['keys']:
            ...
            for inout in in_out_data:
                ...
                if (date_tmp.strftime('%A').lower() in day_off and not check_in) or ('day off' in remarks.lower()):
                    ...
                    emp_sheet.merge_range('B{}:G{}'.format(row + 1, row + 1), date_tmp.strftime('%A') + " Off",
                                          day_off_format)
                else:
                    emp_sheet.write(row, 0, att_date, cell_format_center)
