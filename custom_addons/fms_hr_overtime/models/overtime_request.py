...
class HrOverTime(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', domain=_get_employee_domain, required=True)
    ...
    leave_id = fields.Many2one('hr.holidays',string="Leave ID")
    attchd_copy = fields.Binary('Task File')
    ...
    def _get_days(self):
        ...
        for sheet in self:
            if sheet.date_from and sheet.date_to:
                ...
                diff = fields.Datetime.from_string(sheet.date_to) - fields.Datetime.from_string(sheet.date_from)
    ...
    def send_mail_overtime(self):
        ...
        template_id = ir_model_data.get_object_reference('fms_hr_overtime', 'email_overtime_contract')[1]
        template = self.env['mail.template'].browse(template_id)
        template.write({
            'email_to': users_email or '',
            'body_html':
                'Hello, Employee OverTime request is approved by ceo. please allocated money as soon as possible</br> %s </br></br>Thank You,</p>'% ceo_group.name if ceo_group else ''
        })
    ...
    def approve(self):
        if self.overtime_type_id.type == 'leave':
            ...
            holiday = self.env['hr.holidays'].sudo().create(
                holiday_vals)
            self.leave_id = holiday.id

        # notification to employee :
        self.send_mail_overtime()
        recipient_partners = [(4, self.current_user.partner_id.id)]
...
class HrOverTimeType(models.Model):
    ...
    leave_compute = fields.Many2many('hr.holidays.status', compute="_get_leave_type")
